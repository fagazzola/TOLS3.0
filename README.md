...
- **Calendario** (`src/components/Calendario.jsx`), vista mensual (domingo a sábado) con crear/editar/borrar torneo por día. Cada torneo se asocia a un **campeonato**, elegido con un combo que se llena desde el registro de Campeonatos (ya no es texto libre). El día de pago final se muestra directamente en la cuadrícula con una insignia $ y también se puede editar ahí. Para roles de solo lectura (ej. Jugador) se muestra en cambio como **lista**: arriba, en una franja que queda **fija (sticky)** al hacer scroll de las fechas, el nombre del campeonato vigente (el del próximo torneo pendiente, o si no hay ninguno el del último jugado) con su **estatus** ("No iniciado" / "En curso" / "Terminado", según cuántas fechas ya pasaron), el nombre del jugador del lado derecho, y debajo un resumen con fechas, Main Events, partidas ya jugadas, % de avance del torneo, puntos en el torneo y posición en la tabla — estos dos últimos muestran "Próximamente" porque dependen de los resultados de cada torneo, que todavía no se capturan en ningún lado (eso va a vivir en Game Night). Cada fila de la lista indica fecha, hora, tipo (Regular/Main Event) y si ya fue "Efectuado" o sigue "Pendiente" (comparando la fecha contra hoy) — ya no repite el nombre del campeonato, porque queda fijo arriba; las fechas ya efectuadas muestran también "Posición" y "Puntos", igual en "Próximamente" por ahora. Los datos viven en **Netlify Blobs** (`tols-calendario`), servidos por `netlify/functions/calendario.js` en `/api/calendario`. `src/data/calendario.json` es solo la semilla inicial.
