...
- **Tablero** (`src/components/Tablero.jsx`) — editable en vivo, igual que el Calendario. Premios (por torneo y por campeonato, con % de Rey Killer obligatorio), puntos por posición, cuota de inscripción (por jugador, una vez por campeonato), gastos del campeonato (Tesorero, Pulsera, etc. — se extraen del acumulado y se descuentan del monto a repartir), cobros por torneo (Buy-in y Re-buy obligatorios, Add-on y otros opcionales) y pagos por torneo (Sale en burbuja, Mejor Mano, etc.) — todo parametrizable, con listas de hasta 10 o 15 elementos según la sección. Los datos viven en **Netlify Blobs**, servidos por `netlify/functions/tablero.js` en `/api/tablero`. `src/data/tablero.json` es solo la semilla inicial.
